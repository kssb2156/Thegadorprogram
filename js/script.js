...
function showProductDetails(product) {
    ...
    detailPrice.innerHTML = product.price; // ใช้ price โดยตรง
    ...
    // ตรวจสอบประเภทสินค้า
    if (product.type === 'vip' || product.type === 'cost') {
        // สำหรับสินค้าที่ต้องใส่รหัสผ่าน (VIP และ Cost)
        downloadLink.style.display = 'none';
        passwordFormContainer.style.display = 'block';

        passwordSubmitButton.onclick = () => {
            const correctVipPassword = '123'; // เปลี่ยนรหัสผ่านตรงนี้

            if (passwordInput.value === correctVipPassword) {
                passwordMessage.textContent = 'รหัสผ่านถูกต้อง!';
                passwordMessage.style.color = 'green';
                
                // ให้ปุ่มดาวน์โหลดแสดงขึ้นมาเมื่อรหัสผ่านถูกต้อง
